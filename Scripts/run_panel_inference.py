...
def panel_sort_key(rec: Dict[str, float]):
    # better: lower ci_sum, higher reads, higher singleton_frac, lower fullset_frac
...
def main():
    ...
    best_panel = args.panel if args.panel else (ranked[0]['panel'] if ranked else '')
    runner_up = ranked[1]['panel'] if len(ranked) > 1 else ''

    selected_panels = []
    if args.panel:
        selected_panels = [args.panel]
    else:
        selected_panels = [rec['panel'] for rec in ranked[:max(1, args.ecboot_top)]]
